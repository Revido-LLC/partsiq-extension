...
O usuário inicia o scan manualmente quando quiser. Ao iniciar um novo scan (qualquer URL), aplica **regra de limpeza antes do merge**:

| Peça no carrinho | Ação |
|---|---|
| `pending` ou `error` | **Remove sempre** (independente de URL) |
| `sent` ou `sending` | Preserva sempre |

Após a limpeza, os novos itens são simplesmente concatenados ao cart resultante (apenas itens `sent`/`sending`). Isso garante que cada scan apresente uma lista limpa sem mistura com pendentes de scans anteriores.
...
Recebe `cart: CartItem[]` como prop. Se houver itens com `status === 'sent'`, exibe-os em seção acima da mensagem "no parts found" para que o usuário veja o que já foi enviado.

```
┌─────────────────────────────┐
│ [✂ Crop selection][⊡ Scan] │
├─────────────────────────────┤
│  No parts found on this page│  ← só aparece se cart não tem sent
├─────────────────────────────┤
│  ☑ Filtro de óleo  ✓ Sent  │  ← itens sent (se existirem)
│  ☑ Vela NGK        ✓ Sent  │
├─────────────────────────────┤
│  [+ Add part manually]      │
└─────────────────────────────┘
...
│   ✓ Search finished.        │
│                             │
│   [Check part status in     │  ← botão #00C6B2, navega aba ativa
│    Parts iQ.]               │    via chrome.tabs.update({ url })
│                             │    vehicle → /dash/parts
...
### 2026-04-27 (3 mudanças)

**FinishState — navegar na aba ativa em vez de abrir nova aba**
- `<a target="_blank">` substituído por `<button onClick={() => chrome.tabs.update({ url })}>`
- No sidepanel do Chrome, links normais navegam o próprio painel; `chrome.tabs.update` é o único caminho confiável para navegar a aba ativa

**Cart cleanup — limpar pendentes antes de novo scan**
- `processScan` agora remove todos os itens `pending`/`error` do cart antes de iniciar o merge, independente de URL
- `mergeCart` simplificado: sem filtragem por URL (o cleanup já foi feito antes); apenas concatena `existing + incoming`
- `FallbackState` passa a receber `cart: CartItem[]` e exibe itens `sent` quando busca não encontra nada — evita que peças enviadas sumam da view durante o fallback

**Design — cor de fundo alinhada ao app**
- Background token atualizado de `#FFFFFF` para `#F0F0F0` para corresponder ao sidebar do app PartsIQ
- Aplicado em: `Sidebar.tsx`, `CartState.tsx`, `FallbackState.tsx`, `ScanningState.tsx`
- Spinner overlay de login mantido em `bg-white` (propositalmente)
